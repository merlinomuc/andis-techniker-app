...
import {Cpu,Gauge,Search,ShieldAlert,Tags,Wrench,Camera,Keyboard,CheckCircle2,LoaderCircle,Images,ScanLine} from 'lucide-react';
...
const friendlyErrors={
  ...
  RESEARCH_FAILED_RESPONSE:['Recherche vorübergehend nicht möglich','Die Internetrecherche konnte gerade nicht abgeschlossen werden. Die bereits erkannten Angaben bleiben erhalten. Bitte versuche die Recherche erneut.'],
...
export default function App(){
  ...
  function reset(){tools.clear();setQuery('');setTextValues({query:'',manufacturer:'',model:''});setFocus('auto');setMode('identify');setVision(null);setValues({});setResearch(null);setError(null);setStep('capture');setSource('start');setPendingPicker(null);window.scrollTo({top:0,behavior:'smooth'})}
  function chooseAction(action){
    setError(null);
    if(action==='text'){setSource('text');setPendingPicker(null);return;}
    if(action==='camera' || action==='gallery' || action==='scanner'){
      setSource('image');
      setPendingPicker(action);
    }
  }
  const status=useMemo(()=>step==='capture'?(source==='text'?'TYP SUCHEN':source==='image'?'FOTO LESEN':'START'):(step==='recognition'?'ANGABEN PRÜFEN':'ERGEBNIS'),[step,source]);

  return <><header><div className="brand"><div className="logo"><Wrench/></div><div><h1>ANDIS</h1><p>TECHNIKER-APP</p></div></div><div className="release-badge"><strong>V3.3.2</strong><small>4 OPTIONEN</small></div></header><main>
    <section className="hero card"><div className="system-line"><Cpu/> TECHNISCHER ASSISTENT <b>{status}</b></div><div className="release-strip"><span>NEU</span><b>Vier klare Startoptionen: Kamera, Galerie, Typ oder QR-/Barcode</b></div><h2>Technik einfach erkennen und verstehen.</h2><p>Starte genau so, wie es für dich am einfachsten ist.</p>{step==='capture'&&<div className="entry-grid four"><button className={`entry-card ${source==='image'&&pendingPicker==='camera'?'active':''}`} onClick={()=>chooseAction('camera')}><Camera/><strong>Foto aufnehmen</strong><span>Direkt mit der Kamera fotografieren</span></button><button className={`entry-card ${source==='image'&&pendingPicker==='gallery'?'active':''}`} onClick={()=>chooseAction('gallery')}><Images/><strong>Aus Galerie hochladen</strong><span>Vorhandene Bilder auswählen</span></button><button className={`entry-card ${source==='text'?'active':''}`} onClick={()=>chooseAction('text')}><Keyboard/><strong>Typ eingeben</strong><span>Typ, Modell oder Bestellnummer eingeben</span></button><button className={`entry-card ${source==='image'&&pendingPicker==='scanner'?'active':''}`} onClick={()=>chooseAction('scanner')}><ScanLine/><strong>QR / Barcode scannen</strong><span>Code einscannen und übernehmen</span></button></div>}<div className="pipeline"><span className={step==='capture'?'active':''}>1 Eingeben</span><span className={step==='recognition'?'active':''}>2 Prüfen</span><span className={step==='result'?'active':''}>3 Ergebnis</span></div></section>
    ...
    {step==='result'&&<ResearchResult research={research} recognition={values} onNew={reset}/>}
    <section className="safety card"><ShieldAlert/><div><b>Sicher arbeiten</b><p>KI-Angaben prüfen. Herstellerunterlagen und betriebliche Vorgaben haben Vorrang.</p></div></section></main></>;
}
