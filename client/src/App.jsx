...
import { Camera, Images, ScanLine, Search, Wrench, FileText, RefreshCw, ShieldAlert, X, History, Trash2, Check, LoaderCircle, CircleAlert, ImagePlus, Cpu, Gauge, ChevronRight } from 'lucide-react';
...
function App() {
  ...
  async function analyze() {
    ...
    try {
      const response = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: query.trim(), mode, images }) });
  ...
  function reset() { setQuery(''); setImages([]); setResult(null); setError(''); }
  ...
  return <>
    <header><div className="brand"><div className="logo"><Wrench /></div><div><h1>ANDIS</h1><p>TECHNIKER-APP</p></div></div><span className="version">V1.3</span></header>
    <main>
