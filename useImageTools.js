...
export default function App(){
  ...
  return <><header><div className="brand"><div className="logo"><Wrench/></div><div><h1>ANDIS</h1><p>TECHNIKER-APP</p></div></div><div className="release-badge"><strong>V3.3</strong><small>GESAMTPAKET</small></div></header><main>
    <section className="hero card"><div className="system-line"><Cpu/> TECHNISCHER ASSISTENT <b>{status}</b></div><div className="release-strip"><span>NEU</span><b>Alle Funktionen gebündelt: Typ, Kamera, Galerie, Code und stabile Recherche</b></div><h2>Technik einfach erkennen und verstehen.</h2><p>Typ eingeben, ein Foto aufnehmen oder ein Bild aus der Galerie wählen. Die App sucht zuerst beim Hersteller und erweitert die Recherche nur bei Bedarf.</p>{step==='capture'&&<div className="source-switch"><button className={source==='text'?'active':''} onClick={()=>setSource('text')}><Keyboard/> Typ eingeben</button><button className={source==='image'?'active':''} onClick={()=>setSource('image')}><Camera/> Foto / Galerie</button></div>}<div className="pipeline"><span className={step==='capture'?'active':''}>1 Eingeben</span><span className={step==='recognition'?'active':''}>2 Prüfen</span><span className={step==='result'?'active':''}>3 Ergebnis</span></div></section>
